...
async function registerRoutes() {
  ...
  // 工具处理
  fastify.post("/tools/:toolId/process", async (request, reply) => {
    try {
      const { toolId } = request.params as { toolId: string };
      const { input, options } = request.body as {
        input: string;
        options?: any;
      };
  ...
  // 获取热门工具
  fastify.get("/tools/popular", async (request, reply) => {
    try {
  ...
  // 获取最近工具
  fastify.get("/tools/recent", async (request, reply) => {
    try {
      const { limit } = request.query as { limit?: string };
      const tools = await toolService.getRecentTools(
        limit ? parseInt(limit) : 10
      );
      return { tools };
    } catch (error) {
