...
export class HashGeneratorTool extends BaseTool {
  ...
  getLocalizedContent(language: "zh" | "en" | "ja") {
    if (language === "en") {
      return {
        name: "Hash Generator",
        description: "Generate various hash values (MD5, SHA1, SHA256, SHA512)",
        inputPlaceholder: "Please enter text to generate hash...",
        options: [],
      };
    }
    ...
    if (language === "ja") {
      return {
        name: "ハッシュジェネレーター",
        description: "様々なハッシュ値（MD5、SHA1、SHA256、SHA512）を生成",
  ...
  async process(
    ...
    try {
      ...
      throw new Error(
        `哈希生成失败: ${error instanceof Error ? error.message : "未知错误"}`
      );
    }
  }
