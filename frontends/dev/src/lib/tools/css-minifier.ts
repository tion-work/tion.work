...
export class CSSMinifierTool extends BaseTool {
  ...
  async process(input: string, options: any = {}): Promise<string> {
    ...
    try {
      ...
      throw new Error(`CSS 压缩失败: ${error instanceof Error ? error.message : '未知错误'}`);
    }
  }
  ...
  private minifyCSS(css: string, options: any): string {
    ...
    if (options.minifyValues) {
      ...
      // 压缩 RGB 颜色
      minified = minified.replace(/rgb\s*\(\s*0\s*,\s*0\s*,\s*0\s*\)/g, 'black');
      minified = minified.replace(/rgb\s*\(\s*255\s*,\s*255\s*,\s*255\s*\)/g, 'white');
    }
